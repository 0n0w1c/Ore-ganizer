...
local displayer_fluid_boxes = nil
if mining_drill.input_fluid_box then
    displayer_fluid_boxes = { table.deepcopy(mining_drill.input_fluid_box) }
    for _, fluid_box in pairs(displayer_fluid_boxes) do
        fluid_box.production_type = fluid_box.production_type or "input"
        fluid_box.pipe_covers = nil
        if fluid_box.pipe_connections then
            for _, pipe_connection in pairs(fluid_box.pipe_connections) do
                pipe_connection.hide_connection_info = true
            end
        end
    end
end
